    ...
    while True:
        ...
        try:
            ...
            if len(boxes) > 0:
                ...
                # 跟随逻辑
                if cx < 640 * 0.42:     car_state = 'a'
                elif cx > 640 * 0.58:   car_state = 'd'
                else:
                    if area_ratio < 0.25:   car_state = 'w'
                    elif area_ratio > 0.55: car_state = 's'
                    else:                   car_state = 'stop'
            else:
                car_state = 'stop' 
        except Exception: pass
...
<head>
    ...
    <style>
        ...
        .controls { display: grid; grid-template-columns: repeat(3, 1fr); grid-template-rows: repeat(3, 70px); gap: 15px; max-width: 300px; margin: 20px auto; }
        .btn { background-color: #444; border: none; color: white; font-size: 20px; font-weight: bold; border-radius: 12px; touch-action: none; cursor: pointer; display: flex; align-items: center; justify-content: center; }
...
<body>
    <h2 style="color: #9b59b6; margin-bottom: 5px;">🤖 AI 战车控制台</h2>
...
def handle_action():
    ...
    elif cmd in ['w', 'a', 's', 'd', 'stop']: car_state = cmd
    return "OK", 200
